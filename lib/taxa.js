...
class Taxa {
    ...
    constructor(
        ...
        if ( includePhotos && fs.existsSync( path.join( dataDir, photosFileName ) ) ) {
            const photos = CSV.parseFile( dataDir, photosFileName );
            const photosByName = photos.reduce(
                ( memo, photo ) => {
                    memo[photo.name] = memo[photo.name] || [];
                    memo[photo.name].push( photo );
                    return memo;
                },
                {}
            );
            for ( const taxon of Object.values( this.#taxa ) ) {
                taxon.addPhotos( photosByName[taxon.getName()] || [] );
            }
        }
