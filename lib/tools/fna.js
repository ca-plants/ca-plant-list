...
async function getFNATaxa(toolsDataPath) {
    ...
    const baseURL =
        "http://floranorthamerica.org/Special:Ask/limit=5000/unescape=true/format=html";
    for (const vol of vols) {
        const fn = `${vol}.html`;
        const fullPath = path.join(toolsDataPath, fn);
        if (!Files.exists(fullPath)) {
            const url = baseURL + `/-5B-5BVolume::${vol}-5D-5D`;
            await retrieveFromCloudFlare(url, fullPath);
        }

        const html = scrape.parseFile(fullPath);
        const linksEl = scrape.getSubtree(
            html,
            (e) => scrape.getAttr(e, "class") === "mw-parser-output",
        );
        if (linksEl === undefined) {
            throw new Error();
        }
        const links = scrape.getSubtrees(linksEl, (e) => e.tagName === "a");

        // If there are more than 5000 results, this will need to be updated to retrieve chunks.
        if (Object.entries(links).length >= 5000) {
            throw new Error(`${vol} has more than 5000 results`);
        }

        for (const link of links) {
            const name = scrape.getTextContent(link);
            fnaTaxa.set(name, { name: name });
        }
    }
 ...
 */
async function retrieveFromCloudFlare(url, filePath) {
    const page = await getBrowserPage();

    await page.goto(url);

    await page.waitForSelector("#catlinks", { timeout: 30000 });
