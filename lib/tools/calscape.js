...
async function getCalscapeData(toolsDataDir) {
    ...
    await wb.xlsx.readFile(getExcelFilename(toolsDataDir)).then(function () {
        ...
        for (let index = 0; index < ws.rowCount; index++) {
            ...
            const commonName = getCellValue(row.getCell(4));
            if (!col1 || !commonName) {
                continue;
            }
            data.set(col1, commonName);
        }
    });
 ...
 */
async function retrieveCalscapeFile(toolsDataPath) {
    const url = "https://calscape.org/search/";
    ...
    // options-button only appears if screen is wide enough
    ...
    fs.renameSync(
        ...
        path.join(toolsDataPath, "calscape.xlsx"),
    );
